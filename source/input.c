...
int input_read_parameters_species(struct file_content * pfc,
  ...
  int rescale_N_ur_with_T_ncdm;
  int flag_T_ncdm_for_N_ur;
  double param1, param2, param3, param4, param5;
  double T_ncdm_for_N_ur;
  char string1[_ARGUMENT_LENGTH_MAX_];
  int fileentries;
  ...
  double * pointer3;
  int i;
  double rho_rad, rho_rad_in_J, T_cmb_in_J, gstar_degen, neff_degen, B_yp, f_yp, N_ur2, gap_g, gap_ur;
  double rho_g_prime_in_J;


  sigma_B = 2.*pow(_PI_,5.)*pow(_k_B_,4.)/15./pow(_h_P_,3.)/pow(_c_,2);  // [W/(m^2 K^4) = Kg/(K^4 s^3)]

  /** 1) Omega0_g (photons) and T_cmb */
  /* Read */
  ...
  /* Optionally rescale the residual N_ur with T_ncdm. */
  rescale_N_ur_with_T_ncdm = _FALSE_;
  class_read_flag("rescale_N_ur_with_T_ncdm",
                  rescale_N_ur_with_T_ncdm);

  if (rescale_N_ur_with_T_ncdm == _TRUE_) {

    class_test(class_none_of_three(flag1,flag2,flag3) == _FALSE_,
               errmsg,
               "Do not pass N_ur, Omega_ur, or omega_ur when "
               "rescale_N_ur_with_T_ncdm = yes.");

    class_call(parser_read_double(pfc,
                                  "T_ncdm",
                                  &T_ncdm_for_N_ur,
                                  &flag_T_ncdm_for_N_ur,
                                  errmsg),
               errmsg,
               errmsg);

    class_test(flag_T_ncdm_for_N_ur == _FALSE_,
               errmsg,
               "rescale_N_ur_with_T_ncdm requires T_ncdm.");

    param1 = 0.00441*pow(T_ncdm_for_N_ur/0.71611,4.);
    flag1 = _TRUE_;
  }
  ...
  if (N_ncdm > 0){
    ...
    /** 5.e) Temperatures */
    /* Read */
    /* Begin section calculating the corrected T_ncdm when considering Y decay */
    class_call(parser_read_double(pfc,"maxyr_yp",&param2,&flag2,errmsg),                            
              errmsg,
              errmsg);
    if (flag2 == _TRUE_){
      pba->maxyr_yp = param2;
    ...
    class_call(parser_read_double(pfc,"f_yp",&param4,&flag4,errmsg),
               errmsg,
               errmsg);
    if (flag4 == _TRUE_){
      pba->f_yp = param4;
      if ((pba->f_yp != 0.0) && (pba->N_ncdm > 0)){     //If there is some decay into photons (f!=0), and we are considering a massive neutrino, then correct the T_ncdm
        pba->Omega0_g_prime = pba->Omega0_g/ (1.0 + pba->f_yp*(pba->gap_yp-1.0)*(1.0 + (7./8.)*3.044*pow(4./11., 4./3.) ) );
        class_read_list_of_doubles_or_default("T_ncdm",pba->T_ncdm,pba->T_ncdm_default,N_ncdm);
        for (i=0; i<N_ncdm; i++) {
          pba->T_ncdm[i] = 0.71611*pow(pba->Omega0_g_prime/pba->Omega0_g, 1./4.);
        }
        pba->Omega0_ur_prime = pba->Omega0_g_prime*(7./8.)*N_ur2*pow(4./11., 4./3.);
      }
      else { //if maxyr != 0, but no photons injected, then no need to change T_ncdm
        pba->Omega0_g_prime = pba->Omega0_g;
        class_read_list_of_doubles_or_default("T_ncdm",pba->T_ncdm,pba->T_ncdm_default,N_ncdm);
      }    
    }
    /* End section calculating the corrected T_ncdm when considering Y decay */
  ...
  /** 9) Decaying hidden sector Y particle. Decays into photons and DR */
  /* Read */
  ...
  class_call(parser_read_double(pfc,"maxyr_yp",&param2,&flag2,errmsg),                            
              errmsg,
              errmsg);
  class_call(parser_read_list_of_doubles(pfc,"branch_yp",&int3,&pointer3,&flag3,errmsg),
               errmsg,
               errmsg);
  class_call(parser_read_double(pfc,"f_yp",&param4,&flag4,errmsg),
               errmsg,
               errmsg);
  class_call(parser_read_double(pfc,"m_ncdm",&param5,&flag5,errmsg),
               errmsg,
               errmsg);
  /* Test */
  class_test(((flag1 == _TRUE_) && (flag2 == _FALSE_)),
              errmsg,
              "In input file, you need both Gamma_yp and maxyr_yp, not just one");
  /* Test */
  ...
  class_test(((flag3 == _TRUE_) && (flag4 == _TRUE_)),
              errmsg,
              "In input file, you provided both branch_yp and f_yp. Only use one or the other.");
  ...
  if ((flag3 == _TRUE_) && (flag4 == _FALSE_)){  //if using branch_yp (depricated) rather than f_yp 
    for (i=0; i<int3; i++) {
      pba->branch_yp[i] = pointer3[i];
    }
    free(pointer3);
    if (pba->branch_yp[2]!= 1.0){   //if NOT decaying completely into massless neutrinos. Either fully photons or some mix of photons+neutrinos
      pba->B_yp = pba->branch_yp[2]/pba->branch_yp[0];
      pba->Omega0_ur_prime = ((1.+ pba->B_yp)*pba->Omega0_ur) /(pba->gap_yp + pba->B_yp - ((7./8.)*3.044*pow(4./11., 4./3.)*(1-pba->gap_yp) ));
      pba->Omega0_g_prime = (pba->Omega0_ur_prime*(1.-pba->gap_yp) + pba->Omega0_g*(1.+ pba->B_yp) )/(pba->gap_yp + pba->B_yp) ;
    }
  }
  if ((flag3 == _FALSE_) && (flag4 == _TRUE_)){   //Using f_yp. Should use this
    pba->branch_yp[0] = param4;
    pba->branch_yp[1] = 0.0 ;
    pba->branch_yp[2] = 1.0-param4;
    pba->f_yp = param4;

    /*If the Y decay is injecting photons, then we need to determine the correct initial photon density by scaling back Omega0_g_prime such that the decay 
    results in the correct Omega0_g. In this case, we are not including ncdm (flag5 == _FALSE_), so there is no need to adjust T_ncdm. 
    Assuming 3 massless neutrinos. Therefore, Neff=3.044 sets Omega0_ur_prime. */
    if ((pba->f_yp!= 0.0) && (pba->N_ncdm == 0)){
      //pba->B_yp = pba->branch_yp[2]/pba->branch_yp[0];
      //pba->Omega0_ur_prime = ((1.+ pba->B_yp)*pba->Omega0_ur) /(pba->gap_yp + pba->B_yp - ((7./8.)*3.044*pow(4./11., 4./3.)*(1-pba->gap_yp) ));
      //pba->Omega0_g_prime = (pba->Omega0_ur_prime*(1.-pba->gap_yp) + pba->Omega0_g*(1.+ pba->B_yp) )/(pba->gap_yp + pba->B_yp) ;
      pba->Omega0_g_prime = pba->Omega0_g/ (1.0 + pba->f_yp*(pba->gap_yp-1.0)*(1.0 + (7./8.)*3.044*pow(4./11., 4./3.) ) );
      pba->Omega0_ur_prime = pba->Omega0_g_prime*(7./8.)*3.044*pow(4./11., 4./3.);
      N_ur2 = 3.044;
    }  
    if ((pba->f_yp == 0.0) && (pba->N_ncdm == 0)){
      pba->Omega0_g_prime=pba->Omega0_g;
    ...
    //If there is any decay into photons, AND we are considering ncdm, need to adjust the T_ncdm because we are changing number density of neutrinos at decoupling. 
    // Here we need to use Neff = 2.0328 because the ncdm will contribute to Neff at BBN. 
    /*else if ((pba->branch_yp[2]!= 1.0) && (flag5 == _TRUE_)){
      printf("TEST 2\n");
      //T_cmb_in_J =  (1.6022e-19)*_eV_over_Kelvin_*pba->T_cmb;
      pba->Omega0_g_prime = pba->Omega0_g/ (1.0 + pba->f_yp*(pba->gap_yp-1.0)*(1.0 + (7./8.)*3.044*pow(4./11., 4./3.) ) );
      //pba->Omega_g_prime = ((1.+ pba->B_yp)*pba->Omega0_g) /(pba->gap_yp + pba->B_yp - ((7./8.)*2.0328*pow(4./11., 4./3.))*(1-pba->gap_yp));
      //pba->Omega_ur_prime = ((1.+ pba->B_yp)/(pba->gap_yp + pba->B_yp - ((7./8.)*2.0328*pow(4./11., 4./3.))*(1-pba->gap_yp)) )*pba->Omega0_g*(7./8.)*2.0328*pow(4./11., 4./3.);
      //pba->Omega_g_prime = (pba->Omega_ur_prime*(1.-pba->gap_yp) + pba->Omega0_g*(1.+ pba->B_yp) )/(pba->gap_yp + pba->B_yp) ;
      }
    }  */
  }
  ...
  class_call(parser_read_double(pfc,"Omega_scf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
 class_call(parser_read_double(pfc,"maxyr_yp",&param4,&flag4,errmsg),                            
            errmsg,
            errmsg);
  /* Test */
  ...
  /*If there is Y decay injecting photons, Omega0_ur will be different.*/
  if ((flag4 == _TRUE_) && (pba->f_yp != 0.0)){ 
    gap_g = pba->Omega0_g/pba->Omega0_g_prime;
    gap_ur = 1. + ((1.-pba->f_yp)*(pba->gap_yp-1.)*( 3.044 + (8./7.)*pow(11./4.,4./3.) ))/N_ur2;
    pba->postNeff_yp = (1.0/gap_g)*(3.044+N_ur2*(gap_ur-1.0));
    Omega_tot += pba->Omega0_ur_prime*gap_ur;
  }  
  else if ((flag4 == _TRUE_) && (pba->f_yp == 0.0)){ 
    gap_g = 1.;
    gap_ur = 1. + ((1.-pba->f_yp)*(pba->gap_yp-1.)*( 3.044 + (8./7.)*pow(11./4.,4./3.) ))/N_ur2;
    pba->postNeff_yp = (1.0/gap_g)*(3.044+N_ur2*(gap_ur-1.0));
    Omega_tot += pba->Omega0_ur*gap_ur;
  }  
  else{
...
int input_default_params(struct background *pba,
  ...
  pba->gap_yp=1.0;
  /** 8.4) branching ratio for Y decay into [photons, dark radiaiton, neutrinos] */
  pba->branch_yp[0]=0.0;
  pba->branch_yp[1]=1.0;
  pba->branch_yp[2]=0.0;
  /** or */
