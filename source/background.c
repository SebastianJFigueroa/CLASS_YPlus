...
int background_functions(
  ...
  /* photons */
  /*If there is Y decay and the Y particle is NOT decaying completely into massless neutrinos(i.e. either fully photons or a mix of photons and massless neutrinos, 
  ...
  if (pba->has_yp == _TRUE_) {
    ...
    if (pvecback_B[pba->index_bi_rho_yp] > pow(10.,-24.)) {
      ...
      pvecback[pba->index_bg_rho_yp] = 0.0; // force rho_yp to be zero once sufficiently decayed away in order to avoid numerical issues
    }
  ...
  /* Decay product of Y particle, yr */
  if (pba->has_yr == _TRUE_) {
    pvecback[pba->index_bg_rho_yr] = pvecback_B[pba->index_bi_rho_yr];
    //rho_tot += pvecback[pba->index_bg_rho_yr];
    //p_tot += (1./3.)*pvecback[pba->index_bg_rho_yr];
    //dp_dloga += -(4./3.) * pvecback[pba->index_bg_rho_yr];
    //rho_r += pvecback[pba->index_bg_rho_yr];
  }
  ...
  /*If there is Y decay and the Y particle is injecting any photons, then the photon temperature early on is colder which results in Omega_ur being smaller early on. 
  Use Omega0_ur_prime, and add on top of that the ur that is injected from the Y decay*/
    if ( (pba->has_yp == _TRUE_) && (pba->f_yp != 0.0) ) { //if injecting any photons
      pvecback[pba->index_bg_rho_ur] =  (pba->Omega0_ur_prime * pow(pba->H0,2) / pow(a,4) ) + (pba->branch_yp[2]*pvecback_B[pba->index_bi_rho_yr]) ;
    }
    else if ((pba->has_yp == _TRUE_) && (pba->f_yp==0.0) ) { //if all decay into massless neutrinos
      pvecback[pba->index_bg_rho_ur] =( pba->Omega0_ur * pow(pba->H0,2) / pow(a,4) ) + pvecback_B[pba->index_bi_rho_yr] ;
    }
    else {
...
int background_indices(
  ...
  if (pba->Omega0_ur != 0. || pba->Omega0_ur_prime != 0.)
      pba->has_ur = _TRUE_;
...
int background_solve(
  ...
  /* -> contribution of decaying Y particle and decay product to the critical density today: */
  if (pba->has_yp == _TRUE_) {
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_yp == _TRUE_) {
      ...
      printf("     -> Gamma_yp = %f\n",pba->Gamma_yp);
      printf("     -> max(rho_y/rho_r) = %g\n",pba->maxyr_yp);
      //printf("     -> gap = %f\n",pba->gap_yp);
      printf("     -> photon branching fraction = %g\n",pba->f_yp);
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_yp == _TRUE_) {
    ...
    pvecback_integration[pba->index_bi_rho_yp] =pba->maxyr_yp*pow(tilde,0.5)*exp(0.5*(1-tilde))*(rho_rad);
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->has_yr == _TRUE_) {
      ...
      class_print_species("Ultra-relativistic",yr*(1.-pba->f_yp));
