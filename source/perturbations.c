...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double fracnu,fracg,fracb,fraccdm, fracidm_dr = 0.,fracyp =0.;
  double om;
    ...
    */
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_yp == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_g] += pba->f_yp*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_g])*alpha;
      }
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_)  || (pba->has_idr == _TRUE_)) {
        ...
        if (pba->has_yp == _TRUE_)
            delta_ur += (1.0 - pba->f_yp)*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_ur])*alpha;
      }
...
int perturbations_approximations(
  ...
  double tau_c;
  /* (d) time scale of Y particle decay becoming relevant \f$ \Gamma_Y = H \f$ */
  double gamma_y_over_h;
  ...
  /** - compute time scale of Y particle decay becoming relevant */
  
  if (pba->has_yp == _TRUE_) {
      gamma_y_over_h = pba->Gamma_yp / ppw->pvecback[pba->index_bg_H];
  }
  ...
  if (_scalars_) {
    ...
    /** - ---> (b.2.) if \f$ \kappa' \neq 0 \f$, recombination is not finished: check tight-coupling approximation */
      ...
      if ((tau_c/tau_h < ppr->tight_coupling_trigger_tau_c_over_tau_h) &&
          ...
          ((pba->has_yp == _FALSE_) ||
           (gamma_y_over_h < ppr->tight_coupling_trigger_gamma_y_over_h))) {
        ppw->approx[ppw->index_ap_tca] = (int)tca_on;
      }
      else {
        ppw->approx[ppw->index_ap_tca] = (int)tca_off;
...
int perturbations_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      theta_g += k*k*alpha;
      if (pba->has_yp == _TRUE_) {
        delta_g += pba->f_yp*pvecback[pba->index_bg_a]*pba->Gamma_yp*(pvecback[pba->index_bg_rho_yp]/pvecback[pba->index_bg_rho_g])*alpha;
      }
      ...
      if (pba->has_ur == _TRUE_) {
        ...
        theta_ur += k*k*alpha;
        if (pba->has_yp == _TRUE_) {
          delta_ur += (1.0-pba->f_yp)*pvecback[pba->index_bg_a]*pba->Gamma_yp*(pvecback[pba->index_bg_rho_yp]/pvecback[pba->index_bg_rho_ur])*alpha;
        }
      }
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      // standard term
      -4./3.*(theta_g+metric_continuity)
      // non-standard decay source term from yp -> g
      + (pba->has_yp == _TRUE_
         ? pba->f_yp*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_g])*(y[pv->index_pt_delta_yp]-delta_g+(metric_euler/k2))
         : 0.0);
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
        ...
        dy[pv->index_pt_theta_g] =
            ...
            // non-standard decay source term from yp -> g
            + (pba->has_yp == _TRUE_
               ? pba->f_yp*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_g])*((3./4.)*y[pv->index_pt_theta_yp]-theta_g)
               : 0.0);
    ...
    if (pba->has_ur == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        dy[pv->index_pt_delta_ur] =
          ...
          // non-standard decay source term from yp -> ur
          +(pba->has_yp == _TRUE_
            ? (1.0-pba->f_yp)*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_ur])*(y[pv->index_pt_delta_yp]-y[pv->index_pt_delta_ur]+(metric_euler/k2))
            : 0.0);
        ...
        dy[pv->index_pt_theta_ur] =
          ...
          // non-standard decay source term from yp -> ur
          +(pba->has_yp == _TRUE_
            ? (1.0-pba->f_yp)*a*pba->Gamma_yp*(ppw->pvecback[pba->index_bg_rho_yp]/ppw->pvecback[pba->index_bg_rho_ur])  *((3./4.)*y[pv->index_pt_theta_yp]-y[pv->index_pt_theta_ur])
            : 0.0);
